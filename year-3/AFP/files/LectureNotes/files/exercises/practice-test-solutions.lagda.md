...
+-left-cancellable : (a b c : ℕ)
                   → a + b ≡ a + c
                   → b ≡ c
+-left-cancellable zero    b c h = h
+-left-cancellable (suc a) b c h = +-left-cancellable a b c (ap pred h)
```
...
sub-from-zero zero    = refl zero
sub-from-zero (suc a) = ap pred (sub-from-zero a)

sub'-from-zero : (a : ℕ) → sub' a 0 ≡ 0
sub'-from-zero zero    = refl zero
sub'-from-zero (suc a) = sub'-from-zero a
```
...
pred-sub (suc a) b = ap pred (pred-sub a b)
```
...
sub-agree (suc a) zero    = trans
                             (ap pred (sub-from-zero a))
                             (sym (sub'-from-zero a))
sub-agree (suc a) (suc b) = trans (pred-sub a b) (sub-agree a b)
```
...
is-constant-list-is-decidable n (x :: xs) = ×-preserves-decidability h IH
 where
  h : is-decidable (x ≡ n)
  h = ℕ-has-decidable-equality x n
...
bin-rose-iso = record { bijection = f ; bijectivity = f-is-bijection }
 where
  ...
  g : Rose → Bin
  g (br []) = lf
  g (br (b :: bs)) = nd (g b) (g (br bs))

  g-lemma : (b bs : Rose) → g (nd' b bs) ≡ nd (g b) (g bs)
  g-lemma b (br bs) = refl _

  gf : g ∘ f ∼ id
  gf lf = refl _
  gf (nd l r) =
    g (nd' (f l) (f r))    ≡⟨ g-lemma (f l) (f r) ⟩
    nd (g (f l)) (g (f r)) ≡⟨ ap (λ x → nd x (g (f r))) (gf l) ⟩
    nd l (g (f r))         ≡⟨ ap (λ x → nd l x) (gf r) ⟩
    nd l r                 ∎

  fg : f ∘ g ∼ id
  fg (br []) = refl _
  fg (br (b :: bs)) =
    nd' (f (g b)) (f (g (br bs))) ≡⟨ ap (λ x → nd' x (f (g (br bs)))) (fg b) ⟩
    nd' b (f (g (br bs)))         ≡⟨ ap (λ x → nd' b x) (fg (br bs)) ⟩
    br (b :: bs)                  ∎
