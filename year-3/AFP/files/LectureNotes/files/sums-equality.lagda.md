...
```agda
Σ-≡-fst : {X : Type} {A : X → Type} {(x , a) (y , b) : Σ x ꞉ X , A x}
        → (x , a) ≡ (y , b) → x ≡ y
...
So the functions `from-Σ-≡` and `to-Σ-≡` give a complete characterization of equality in `Σ` types as a logical equivalence.
...
Σ-≡ = from-Σ-≡ , to-Σ-≡
```
But we actually have an isomorphism:
```
open import isomorphisms
open import products

Σ-≡-iso : {X : Type} {A : X → Type} {(x , a) (y , b) : Σ A}
        → ((x , a) ≡ (y , b)) ≅ (Σ e ꞉ x ≡ y , transport A e a ≡ b)
Σ-≡-iso {X} {A} {x , a} {y , b} = Isomorphism from-Σ-≡ (Inverse to-Σ-≡ to-from from-to)
 where
  to-from : to-Σ-≡ ∘ from-Σ-≡ ∼ id
  to-from (refl (x , a)) = refl (refl (x , a))

  from-to : from-Σ-≡ ∘ to-Σ-≡ ∼ id
  from-to (refl x , refl .(transport A (refl x) a)) =
     refl (refl x , refl (transport A (refl x) a))
```
